             ...
             BITMAPINFO **info)    /* O - Bitmap information */
{
    ...
    if (fread(&header, sizeof(BITMAPFILEHEADER), 1, fp) < 1)
    {
        ...
        LOG_ERROR("Unable to read the BMP file header"); 
        fclose(fp);
        return (NULL);
    }
    ...
    if ((*info = (BITMAPINFO *)malloc(infosize)) == NULL)
    {
        ...
        LOG_ERROR("Couldn't allocate " << infosize << " bytes to hold bmp image in memory"); 
        fclose(fp);
        return (NULL);
    }
    ...
    if (fread(*info, 1, infosize, fp) < infosize)
    {
        ...
        LOG_ERROR("Couldn't read BMP header"); 
        free(*info);
        fclose(fp);
        return (NULL);
    }
    ...
    if (fread(bits, 1, bitsize, fp) < bitsize)
    {
        ...
        LOG_ERRROR("Unable to read bitmap bits"); 
        free(*info);
        free(bits);
        fclose(fp);
        return (NULL);
