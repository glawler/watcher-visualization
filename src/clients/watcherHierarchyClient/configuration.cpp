...
namespace watcherHierarchyClient {
    ...
    bool validateConfiguration() 
    {
        ...
        bool retVal=true;
        if (!vm.count("server"))  {
            cerr << "Server is not an optional argument." << endl;
            retVal=false;
        }
        if (!vm.count("server"))  {
            cerr << "Hierachy daemon address is not an optional argument." << endl;
            retVal=false;
        }
    ...
    bool loadConfiguration(int argc, char **argv)
    {
        ...
        config.add_options()
            ...
            ("hierarchyDaemonAddress,u", value<string>()->default_value("localhost"), "Where the hierarchy daemon is running.")
