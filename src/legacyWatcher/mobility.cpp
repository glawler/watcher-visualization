...
#include "gmcluster.h"   /* Should not reall y be here, but we need to send PACKET_GROUP_MOBILTY_GPS messages for teh gps mobility */
...
static const char *rcsid __attribute__ ((unused)) = "$Id: mobility.cpp,v 1.41 2007/07/23 17:02:15 dkindred Exp $";
...
static void nodeMobilityGPSDataInternal(manetNode *us, void *)
{
	...
	/* send the WatcherGPS message to anyone who is listening like, say, oh, I don't know, gmcluster? */
	WatcherGPS location = { lat, lon, alt, ts*1000 }; 	
	packet *p=packetMalloc(us, sizeof(WatcherGPS)); 
	watcherGPSMarshal(p->data, sizeof(WatcherGPS), &location); 
	p->type=PACKET_GROUP_MOBILTY_GPS;
	p->dst=us->addr; 
	p->ttl=0; 
