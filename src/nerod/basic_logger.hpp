...
/// typedef.
template <typename Service> class basic_logger : private boost::noncopyable
{
public:
  ...
  explicit basic_logger(boost::asio::io_service& io_service, const std::string& identifier)
    : service_(boost::asio::use_service<Service>(io_service)), impl_(service_.null())
  {
  ...
  void use_file(const std::string& file, bool append=true)
  {
    service_.use_file(impl_, file, append);
  }
